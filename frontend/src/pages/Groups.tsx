...
export function Groups() {
  ...
  const [selectedId, setSelectedId] = useState(mockChats[0]?.id ?? null);

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return mockChats;
    return mockChats.filter((c) => c.title.toLowerCase().includes(q) || c.username?.toLowerCase().includes(q));
  }, [query]);

  const selected = mockChats.find((c) => c.id === selectedId) ?? null;
      ...
      <Card className="lg:col-span-3">
        {selected ? (
          <CardBody className="space-y-6">
            ...
            <div>
              <h3 className="text-xs font-semibold uppercase tracking-wider text-slate-500">Audience lifecycle distribution</h3>
              <p className="mt-0.5 text-xs text-slate-400">Based on the last-seen status Telegram exposes for each member. Members who hide their status are shown as "hidden".</p>
              <div className="mt-3 space-y-3">
                {[
                  { label: 'Online / recently active', value: 34, tone: 'success' as const },
                  { label: 'Active within a week', value: 48, tone: 'brand' as const },
                  { label: 'Inactive or hidden', value: 18, tone: 'slate' as const },
                ].map((row) => (
                  <div key={row.label}>
          ...
          <CardBody>Select a community to see details.</CardBody>
        )}
      </Card>
    </div>
  );
